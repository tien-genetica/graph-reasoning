...
def df2Graph(
    ...
    """
    Convert dataframe to graph using concurrent processing for LLM calls.

    Args:
        dataframe: Input dataframe with text chunks
        generate: LLM generation function
        repeat_refine: Number of refinement iterations
        verbatim: Verbose output flag
        max_workers: Maximum number of concurrent threads

    Returns:
        List of graph concepts
    """

    def process_row(row):
        """Process a single row with graphPrompt"""
        return graphPrompt(
            row.text,
            generate,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_row = {
            executor.submit(process_row, row): idx for idx, row in dataframe.iterrows()
        }
        ...
        for future in as_completed(future_to_row):
            try:
                ...
                if result is not None:  # Filter out None results (equivalent to dropna)
                    results.append(result)
            except Exception as exc:
                row_idx = future_to_row[future]
                print(f"Row {row_idx} generated an exception: {exc}")
                if verbatim:
    ...
    if len(results_series) > 0:
        concept_list = np.concatenate(results_series).ravel().tolist()
    else:
        concept_list = []

    if verbatim:
        print(
            f"Processed {len(dataframe)} chunks concurrently, got {len(results)} valid results"
        )
...
def graphPrompt(
    input: str,
    generate,
    metadata={},  # model="mistral-openorca:latest",
    repeat_refine=0,
    verbatim=False,
):
    ...
    if repeat_refine > 0:
        for rep in tqdm(range(repeat_refine)):
            ...
            USER_PROMPT = (
                f"Insert new triplets into the original ontology. Read this context: ```{input}```."
                f"Read this ontology: ```{response}```"
            ...
            if verbatim:
                print(
                    f"---------------------\nAfter refine {rep}/{repeat_refine}: ",
                    response,
                )
...
def make_graph_from_text(
    ...
    outputdirectory = Path(
        f"./{data_dir}/"
    )  # where graphs are stored from graph2df function
    ...
    if regenerate:
        ...
        dfg1.to_csv(
            outputdirectory / f"{graph_root}_graph_clean.csv",  # sep="|", index=False
        )
        df.to_csv(
            outputdirectory / f"{graph_root}_chunks_clean.csv",  # sep="|", index=False
        )
    else:
...
def add_new_subgraph_from_text(
    txt,
    generate,
    node_embeddings,
    tokenizer,
    ...
    do_simplify_graph=True,  # whether or not to simplify, uses similiraty_threshold defined above
    return_only_giant_component=False,
    save_common_graph=False,
    G_to_add=None,
    graph_GraphML_to_add=None,
    max_workers=4,  # Number of concurrent threads for LLM calls
):
    ...
    res = None
    assert not (
        G_to_add is not None and graph_GraphML_to_add is not None
    ), "G_to_add and graph_GraphML_to_add cannot be used together. Pick one or the other to provide a graph to be added."
    ...
    try:
        ...
        if (
            graph_GraphML_to_add == None and G_to_add == None
        ):  # make new if no existing one provided
            print("Make new graph from text...")
            _, graph_GraphML_to_add, G_to_add, _, _ = make_graph_from_text(
                txt,
                generate,
            ...
            if verbatim:
                print(
                    "Instead of generating graph, loading it or using provided graph...(any txt data provided will be ignored...)"
                )
    ...
    try:
        ...
        if G_to_add != None:
            ...
            if verbatim:
                print(
                    "Using provided graph to add (any txt data provided will be ignored...)"
                )
        else:
            if verbatim:
        ...
        if save_common_graph:
            ...
            try:
                ...
                graph_GraphML = (
                    f"{data_dir_output}/{graph_root}_common_nodes_before_simple.graphml"
                )
        ...
        if return_only_giant_component:
            ...
            connected_components = sorted(
                nx.connected_components(G_new), key=len, reverse=True
            )
