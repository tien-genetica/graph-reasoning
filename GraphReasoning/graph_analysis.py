...
def heuristic_path_with_embeddings(
    G,
    embedding_tokenizer,
    embedding_model,
    source,
    ...
    source = find_best_fitting_node_list(
        source, node_embeddings, embedding_tokenizer, embedding_model, 5
    )[0][0].strip()
    target = find_best_fitting_node_list(
        target, node_embeddings, embedding_tokenizer, embedding_model, 5
    )[0][0].strip()
    ...
    def sample_path(current, visited):
        ...
        while current != target:
            neighbors = [
                (neighbor, heuristic(neighbor, target))
            ...
            if not neighbors:
                ...
                if len(path) > 1:
                    visited.add(
                        path.pop()
                    )  # Mark the dead-end node as visited and remove it from the path
                ...
                top_neighbors = (
                    neighbors[:top_k] if len(neighbors) > top_k else neighbors
                )
...
def find_shortest_path(
    G, source="graphene", target="complexity", verbatim=True, data_dir="./"
):
...
def find_shortest_path_with2hops(
    G,
    source="graphene",
    target="complexity",
...
def find_path(
    ...
    path, path_graph, shortest_path_length, fname, graph_GraphML = (
...
def describe_communities(G, N=10):
    ...
    for i, (comm_id, nodes) in enumerate(sorted_communities, start=1):
        ...
        print(
            f"Community {i} (ID {comm_id}) with {len(nodes)} nodes, key nodes (Node ID: Degree):"
        )
...
def describe_communities_with_plots(G, N=10, N_nodes=5, data_dir="./"):
    ...
    for i, nodes in enumerate(all_communities_sorted[:N], start=0):
        ...
        bars = ax.bar(
            range(len(key_nodes)), key_degrees, tick_label=key_nodes, color="lightgreen"
        )
...
def describe_communities_with_plots_complex(G, N=10, N_nodes=5, data_dir="./"):
    ...
    for nodes in all_communities_sorted:
        ...
        top_betweenness_values.append(
            np.mean(top_betweenness) if top_betweenness else 0
        )
    ...
    axs[1, 1].bar(
        range(len(top_betweenness_values)), top_betweenness_values, color="salmon"
    )
    axs[1, 1].set_title(
        "Average Betweenness Centrality for Top Nodes in Each Community"
    )
...
def print_path_with_edges_as_list_multigraph(G, path, keywords_separator=" --> "):
    ...
    for i in range(len(path) - 1):
        ...
        if edge_data:
            ...
            edge_title = " or ".join(
                titles
            )  # This handles multiple edges by joining their titles with " or "
        else:
            edge_title = "No title"
...
def find_path_and_reason(
    ...
    similarity_fit_ID_node_1=0,
    similarity_fit_ID_node_2=0,  # whoch path to include 0=only best, 1 onlysecond best, etc.
    save_files=True,
    data_dir="./",
    ...
    if contains_phrase(graph_analysis_type, "path and relations"):
        ...
        if visualize_paths_as_graph:
            ...
            if verbatim:
                print(
                    "Raw path for graph visualization: ",
                    path_list_for_vis,
                    "original: ",
                    path,
                )

            G_vis = visualize_paths_pretty(
                [path_list_for_vis],
                filename=f"{best_node_1}_{best_node_2}.svg",
    ...
    if contains_phrase(graph_analysis_type, "nodes and relations"):
        ...
        task = (
            task
            + f"{inst_prepend}Consider this list of nodes and relations in a knowledge graph:\n\nFormat: node_1, relationship, node_2\n\nThe data is:\n\n{join_strings_newline( node_list)}\n\n"
        )
    ...
    response = generate(
        system_prompt=system_prompt,  # local_llm=local_llm,
        prompt=task,
...
def find_path_with_relations_and_reason_combined(
    G,
    node_embeddings,
    tokenizer,
    model,
    ...
    data_dir="./",
    save_files=False,  # whether or not to make HTML of each graph
    ...
    if include_all_possible:
        ...
        for start_id in range(num_paths):
            for end_id in range(num_paths):
                ...
                paths_details.extend(
                    process_path_combination(
                        G,
                        node_embeddings,
                        tokenizer,
                        model,
                        ...
                        start_id,
                        end_id,
                        include_keywords_as_nodes,
                        data_dir,
                        save_files,
                        visualize_paths_as_graph,
                        display_graph=display_graph,
                        words_per_line=words_per_line,
                    )
                )
    ...
    for i, (start_id, end_id, path_list, path_list_vis, path_list_string) in enumerate(
        paths_details
    ):
    ...
    if visualize_paths_as_graph:
        G_vis = visualize_paths_and_save_with_labels(
            complete_path_list,
            filename=f"joined_{keyword_1[:20]}_{keyword_2[:20]}.svg",
            display_graph=display_graph,
            data_dir=data_dir,
            scale=1.25,
            node_size=4000,
        ...
        nx.write_graphml(
            G_vis, f"{data_dir}/joined_{keyword_1[:20]}_{keyword_2[:20]}.graphml"
        )
        make_HTML(
            G_vis,
            data_dir=data_dir,
            graph_root=f"joined_{keyword_1[:20]}_{keyword_2[:20]}",
        )

        G_vis = visualize_paths_unique(
            complete_path_list,
            filename=f"joined_unique_{keyword_1[:20]}_{keyword_2[:20]}.svg",
            display_graph=display_graph,
            data_dir=data_dir,
            scale=1.25,
            node_size=4000,
        ...
        nx.write_graphml(
            G_vis, f"{data_dir}/joined_unique_{keyword_1[:20]}_{keyword_2[:20]}.graphml"
        )

        make_HTML(
            G_vis,
            data_dir=data_dir,
            graph_root=f"joined_unique_{keyword_1[:20]}_{keyword_2[:20]}",
        )
...
def process_path_combination(
    ...
    (
        ...
        similarity_fit_ID_node_2=end_id,
        data_dir=data_dir,
        save_files=save_files,
    )

    if visualize_paths_as_graph:
        path_new, _ = print_path_with_edges_as_list(
            G, path, keywords_separator=keywords_separator
        )
    ...
    path_list, path_list_string = print_path_with_edges_as_list(
        G, path, keywords_separator=keywords_separator
    )
    path_list_vis, _ = print_path_with_edges_as_list(
        G, path_new, keywords_separator=keywords_separator
    )
...
def visualize_paths_unique(
    paths,
    filename="graph_distinguish.svg",
    display_graph=False,
    data_dir="./",
    scale=1.25,
    node_size=4000,
    ...
    for path_index, path in enumerate(paths):
        for i in range(0, len(path) - 2, 2):
            relationship = split_label(
                path[i + 1], words_per_line=words_per_line
            ).lower()
            # Determine if the node is a unique start/end node
            start_node_key = (
                f"{path[i]}_{path_index}" if path[i] not in all_starts_ends else path[i]
            )
            end_node_key = (
                f"{path[i+2]}_{path_index}"
                if path[i + 2] not in all_starts_ends
                else path[i + 2]
            )
    ...
    pos = nx.spring_layout(G, seed=42, k=3.0 / scale, iterations=150)

    plt.figure(figsize=(15 * scale, 10 * scale))

    # Determine node colors: start/end nodes one color, others a different color
    node_colors = [
        "lightgreen" if node in all_starts_ends else "skyblue" for node in G.nodes()
    ]
    ...
    nx.draw_networkx_labels(
        G, pos, labels=node_labels, font_size=10 * scale, font_color="darkblue"
    )
    edge_labels = nx.get_edge_attributes(G, "label")
    nx.draw_networkx_edge_labels(
        G, pos, edge_labels=edge_labels, font_size=10 * scale, font_color="red"
    )
...
def visualize_paths_and_save_with_labels(
    paths,
    filename="graph.svg",
    display_graph=False,
    data_dir="./",
...
def visualize_paths_pretty(
    paths,
    filename="graph_pretty.svg",
    display_graph=False,
    data_dir="./",
    scale=1.25,
    node_size=4000,
    ...
    for path in paths:
        for i in range(
            0, len(path) - 2, 2
        ):  # Step by 2 to process node, edge, node triples
            ...
            if (
                i + 2 == len(path) - 1 and node_end not in node_positions
    ...
    edge_labels = nx.get_edge_attributes(G, "label")
    nx.draw_networkx_edge_labels(
        G, pos, edge_labels=edge_labels, font_color="red", font_size=10 * scale
    )
...
def visualize_paths_pretty_spiral(
    paths,
    filename="graph_pretty.svg",
    display_graph=False,
    data_dir="./",
    ...
    for i in range(num_shells):
        shells.append(
            [node for path in paths for node in path[i * 2 :: 2] if i * 2 < len(path)]
        )
...
def analyze_and_visualize_community_structure(
    G, data_dir="./", root="graph", algorithm="greedy_modularity"
):
    ...
    avg_intra_community_edges = (
        intra_community_edges / len(communities) if communities else 0
    )
    avg_inter_community_edges = (
        inter_community_edges / len(communities) if communities else 0
    )
    ...
    axs[2].bar(
        ["Avg Intra-Community Edges", "Avg Inter-Community Edges"],
        [avg_intra_community_edges, avg_inter_community_edges],
    ...
    # Save the figure
    fig.savefig(
        os.path.join(data_dir, f"{root}_community_analysis_{algorithm}.svg"),
        format="svg",
    )
...
def visualize_community_structure_nolabels(G, title="Graph", ax=None):
    ...
    for community, color in zip(communities, colors):
        nx.draw_networkx_nodes(
            G, pos, nodelist=community, node_color=[color], ax=ax, node_size=50
        )  # Adjust node_size if necessary
...
def visualize_community_structure(G, title="Graph", ax=None):
    ...
    label_pos = {
        k: (v[0], v[1] + 10) for k, v in pos.items()
...
def find_isomorphism_and_map_edges(G1, G2, edge_label="title"):
    ...
    if matcher.is_isomorphic():
        ...
        for u, v in G1.edges():
            ...
            edge_label_g2 = (
                G2[u_mapped][v_mapped].get(edge_label, "Label not found")
            ...
            edge_mapping[(u, v)] = (
                (u_mapped, v_mapped),
                G1[u][v].get(edge_label, "Label not found"),
                edge_label_g2,
            )
        ...
        for edge_g1, (edge_g2, label_g1, label_g2) in edge_mapping.items():
            print(
                f"{edge_g1} ('{label_g1}') in G1 maps to {edge_g2} ('{label_g2}') in G2"
            )
        return node_mapping, edge_mapping
    else:
        print("The graphs are not isomorphic.")
...
def visualize_community_structure_in_giant_component(
    G1,
    G2,
    title1="Subgraph 1 Giant Component",
    title2="Subgraph 2 Giant Component",
    filename="plot.svg",
    data_dir="./",
    root="graph",
):
    ...
    node_mapping, edge_mapping = find_isomorphism_and_map_edges(
        get_giant_component(G1), get_giant_component(G2), edge_label="title"
    )
    ...
    for i, G in enumerate([G1, G2]):
        ...
        pos = graphviz_layout(
            G_giant, prog="neato"
        )  # Using 'neato' for improved layout
        ...
        for community in communities:
            ...
            community_node_sizes = [
                degrees[n] * 500 for n in community_nodes
            ]  # Apply scaling for community nodes
            nx.draw_networkx_nodes(
                G_giant,
                pos,
        ...
        edge_labels = {
            edge: split_label_forgraph(label)
            for edge, label in nx.get_edge_attributes(G_giant, "title").items()
        }
        nx.draw_networkx_edge_labels(
            G_giant, pos, edge_labels=edge_labels, ax=axs[i], font_size=8
        )

        # Preprocess node labels similarly
        node_labels = {
            node: split_label_forgraph(data.get("label", node))
...
def find_and_save_isomorphic_subgraphs_with_communities(
    G1,
    G2,
    data_dir,
    root="graph",
    ...
    for iso_map in tqdm(matcher.subgraph_isomorphisms_iter()):
        ...
        avg_degree1 = sum(dict(subgraph1.degree()).values()) / float(
            subgraph1.number_of_nodes()
        )
        avg_degree2 = sum(dict(subgraph2.degree()).values()) / float(
            subgraph2.number_of_nodes()
        )
        ...
        if (
            len(largest_component1) < min_component_size
            and len(largest_component2) < min_component_size
        ):
    ...
    for i, (score, subgraph1, subgraph2) in enumerate(scores_subgraphs[:top_n], 1):
        ...
        nx.write_graphml(
            subgraph1, os.path.join(data_dir, f"top_{i}_subgraph1_{root}.graphml")
        )
        nx.write_graphml(
            subgraph2, os.path.join(data_dir, f"top_{i}_subgraph2_{root}.graphml")
        )

        fig, axs = plt.subplots(1, 2, figsize=(16, 8))
        visualize_community_structure(
            subgraph1, f"Top {i} Subgraph 1 Communities (Score: {score:.2f})", ax=axs[0]
        )
        visualize_community_structure(
            subgraph2, f"Top {i} Subgraph 2 Communities (Score: {score:.2f})", ax=axs[1]
        )

        visualization_filename = os.path.join(
            data_dir, f"top_{i}_subgraph_communities_{root}.svg"
        )
        plt.tight_layout()
        ...
        visualize_community_structure_in_giant_component(
            subgraph1,
            subgraph2,
            title1=f"Top {i} Subgraph 1 Giant Component",
            title2=f"Top {i} Subgraph 2 Giant Component",
            filename=f"{data_dir}/top_{i}_subgraph1-2_giant_{root}.svg",
            data_dir=data_dir,
            root="graph",
        )

        nx.write_graphml(
            get_giant_component(subgraph1),
            os.path.join(data_dir, f"top_{i}_subgraph1_GIANT_F_{root}.graphml"),
        )
        nx.write_graphml(
            get_giant_component(subgraph2),
            os.path.join(data_dir, f"top_{i}_subgraph2_GIANT_F_{root}.graphml"),
        )

    print(
        f"Top {top_n} isomorphic subgraphs processed and saved with community analysis."
    )
...
def calculate_bridging_coefficient(G):
    ...
    for node in G.nodes():
        ...
        bridging_coefficient[node] = (1 / degrees[node]) * sum(
            (1 / degrees[neighbor]) for neighbor in neighbors
        )
...
def remove_top_n_bridging_centrality(G, top_N):
    ...
    bridging_centrality = {
        node: betweenness_centrality[node] * bridging_coefficient[node]
        for node in G.nodes()
    }

    # Sort nodes by bridging centrality and select the top N nodes
    top_n_nodes = sorted(
        bridging_centrality, key=bridging_centrality.get, reverse=True
    )[:top_N]
...
def include_top_n_betweenness_centrality(G, top_N=5):
    ...
    top_n_nodes = sorted(
        betweenness_centrality, key=betweenness_centrality.get, reverse=True
    )[:top_N]
...
def add_bridging_and_centrality_attributes(G):
    ...
    for node in G_new.nodes():
        G_new.nodes[node]["bridging_centrality_nwx"] = (
            betweenness_centrality[node] * bridging_coefficient[node]
        )
...
def save_graph_with_bridging_and_centrality_attributes(
    G, filename="graph_with_bridging_centrality.graphml"
):
    G_new = add_bridging_and_centrality_attributes(
        G
    )  # Add the bridging centrality attribute to each node
    nx.write_graphml(G_new, filename)  # Save the graph to a GraphML file


def use_graph_and_reason_over_triples(
    path_graph,
    generate,
    keyword_1="music and sound",
    # local_llm=None,
    ...
    if include_keywords_as_nodes:
        task = (
            task
            + f"The following is a graph provided from an analysis of relationships between the concepts of {keyword_1} and {keyword_2}.\n\n"
        )
    task = (
        task
        + f"{inst_prepend}Consider this list of nodes and relations in a knowledge graph:\n\nFormat: node_1, relationship, node_2\n\nThe data is:\n\n{join_strings_newline( node_list)}\n\n"
...
def heuristic_path_with_embeddings_with_randomization_waypoints(
    G,
    embedding_tokenizer,
    embedding_model,
    source,
    ...
    if verbatim:
        print("Original: ", source, "-->", target)
    source_list = find_best_fitting_node_list(
        source, node_embeddings, embedding_tokenizer, embedding_model, 5
    )
    target_list = find_best_fitting_node_list(
        target, node_embeddings, embedding_tokenizer, embedding_model, 5
    )
    ...
    def add_random_waypoints(G, path, num_waypoints):
        ...
        for node in path:
            all_neighbors.extend(
                [neighbor for neighbor in G.neighbors(node) if neighbor not in path]
            )
        ...
        for waypoint in waypoints:
            waypoint_path = nx.shortest_path(
                G, source=new_path[-1], target=waypoint, weight="weight"
            )
            new_path.extend(
                waypoint_path[1:]
            )  # Add the path to the waypoint, avoid duplication of the last node

        final_leg = nx.shortest_path(
            G, source=new_path[-1], target=target, weight="weight"
        )
        new_path.extend(
            final_leg[1:]
        )  # Add the final leg to the target, avoid duplication of the last node
        return new_path

    def sample_path_with_randomness(
        source, target, randomness_factor, num_random_waypoints
    ):
        if randomness_factor == 0:
            ...
            try:
                ...
                print(
                    f"No path found between {source} and {target} using Dijkstra's algorithm."
                )
                return None
        else:
    ...
    path = sample_path_with_randomness(
        source, target, randomness_factor, num_random_waypoints
    )
    if path is None:
        print(f"No path found between {source} and {target}")
            ...
            for neighbor in G.neighbors(node):
                ...
                for second_hop_neighbor in G.neighbors(neighbor):
                    if not subgraph.has_node(second_hop_neighbor):
                        subgraph.add_node(
                            second_hop_neighbor, **G.nodes[second_hop_neighbor]
                        )
                    if G.has_edge(neighbor, second_hop_neighbor):
                        subgraph.add_edge(
                            neighbor,
                            second_hop_neighbor,
                            **G.edges[neighbor, second_hop_neighbor],
                        )
...
def analyze_basic_properties(G, original_properties=None):
    ...
    if original_properties:
        ...
        properties["original_degrees"] = {
            node: original_degree_dict.get(node, 0) for node in G.nodes()
        }
...
def spectral_analysis(G):
    ...
    spectral_gap = (
        eigenvalues[-1] - eigenvalues[-2] if len(eigenvalues) > 1 else 0
    )  # Largest - second largest eigenvalue
...
def plot_analysis_trends(
    analysis_results,
    num_waypoints_range,
    xlabel="Number of Random Waypoints",
    include_avg_clustering=False,
):
    num_nodes = [
        result["basic_properties"]["number_of_nodes"] for result in analysis_results
    ]
    num_edges = [
        result["basic_properties"]["number_of_edges"] for result in analysis_results
    ]
    density = [result["basic_properties"]["density"] for result in analysis_results]
    avg_clustering = [
        result["basic_properties"]["average_clustering"] for result in analysis_results
    ]
    avg_degree = [
        result["basic_properties"]["average_degree"] for result in analysis_results
    ]
