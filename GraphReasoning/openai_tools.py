...
def reason_over_image_OpenAI(
    ...
    prompt="Carefully analyze this graph. Be creative and synthesize new research ideas to build sustainable mycelium materials.",
...
def reason_over_image_and_graph_via_triples(
    ...
    task = (
        task
        + f"Consider this list of nodes and relations in a knowledge graph:\n\nFormat: node_1, relationship, node_2\n\nThe data is:\n\n{join_strings_newline( node_list)}\n\n"
    )
...
"""
    ...
    for index, image_dict in enumerate(generation_response.data):
        ...
        image_file = os.path.join(
            image_dir_name, f"generated_image_{time_part}_{response[:32]}_{index}.png"
        )
