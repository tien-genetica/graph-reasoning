    ...
    for node, embedding in embeddings.items():
        ...
        if len(min_heap) < N_samples:
            ...
            if similarity > min_heap[0][0]:
                ...
                heapq.heappush(
                    min_heap, (similarity, node)
                )  # Add the current node and similarity
...
def visualize_embeddings_2d_pretty(embeddings, n_clusters=3, data_dir="./"):
    ...
    plt.savefig(
        f"{data_dir}/node_embeddings_2d_clusters_pretty.svg"
...
def visualize_embeddings_2d_pretty_and_sample(
    embeddings,
    n_clusters=3,
    n_samples=5,
    data_dir="./",
    ...
    for cluster in range(n_clusters):
        ...
        plt.scatter(
            ...
            s=s,
            cmap=palette,  # alpha=0.7, edgecolors='w', s=100, cmap=palette)
        )
...
def visualize_embeddings_with_gmm_density_voronoi_and_print_top_samples(
    embeddings, n_clusters=5, top_n=3, data_dir="./", s=50
):
    # Extract the embedding vectors
    ...
    for i in range(n_clusters):
        plt.scatter(
            vectors_2d[labels == i, 0],
            vectors_2d[labels == i, 1],
            s=s,
            label=f"Cluster {i}",
        )
...
def graph_statistics_and_plots(G, data_dir="./"):
    ...
    plt.hist(
        degrees, bins=range(min(degrees), max(degrees) + 1), alpha=0.75, color="blue"
    )
...
def graph_statistics_and_plots_for_large_graphs(
    G,
    data_dir="./",
...
def graph_Louvain(G, graph_GraphML=None, palette="hls"):
...
def simplify_graph_simple(
    ...
    for i, j in tqdm(zip(*to_merge)):
        if i != j:  # ignore self-similarity
            ...
            if use_llm:
                ...
                node_to_keep = simplify_node_name_with_llm(
                    node_to_keep,
                    generate,
                    max_tokens=max_tokens,
                    temperature=temperature,
    ...
    recalculated_embeddings = regenerate_node_embeddings(
        new_graph, nodes_to_recalculate, tokenizer, model
    )
...
def simplify_graph(
    ...
    for i, j in tqdm(zip(*to_merge), total=len(to_merge[0])):
        if (
            i != j and nodes[i] not in merged_nodes and nodes[j] not in merged_nodes
            ...
            try:
                ...
                merged_nodes.add(
                    node_to_merge
                )  # Mark the merged node to avoid duplicate handling
            except:
                print(end="")
    if verbatim:
        print("Now relabel. ")
...
def return_giant_component_G_and_embeddings(
    G_new, node_embeddings, tokenizer=None, model=None, verbatim=False
):
    connected_components = sorted(nx.connected_components(G_new), key=len, reverse=True)
    G_new = G_new.subgraph(connected_components[0]).copy()
    if tokenizer is not None and model is not None:
        ...
        # Filter embeddings to only include nodes that are still in the graph
        node_embeddings = {
            node: emb for node, emb in node_embeddings.items() if node in G_new.nodes()
        }
...
def get_list_of_graphs_and_chunks(
    graph_q="graph_*_graph_clean.csv",
    chunk_q="graph_*_chunks_clean.csv",
    data_dir="./",
    verbatim=False,
):
...
def get_text_associated_with_node(
    G,
    node_identifier="bone",
):
    ...
    if "texts" in G.nodes[node_identifier]:
        ...
        concatenated_texts = "; ".join(
            texts
        )  # Assuming you want to concatenate the texts
...
def print_nodes_and_labels(G, N=10):
    ...
    for node1, node2, data in list(G.edges(data=True))[:10]:
        edge_title = data.get(
            "title"
        )  # Replace 'title' with the attribute key you're interested in
...
def make_graph_from_text_withtext(
    graph_file_list,
    chunk_file_list,
    ...
    for idx in tqdm(
        range(idx_start, min(len(graph_file_list), N_max)), desc="Processing graphs"
    ):
        try:
            ...
            graph_df = (
                graph_df.groupby(["node_1", "node_2"])
    ...
    chunk_id_to_text = pd.Series(
        all_texts_df.text.values, index=all_texts_df.chunk_id
    ).to_dict()
    ...
    for node1, node2, data in tqdm(
        G_total.edges(data=True), desc="Mapping texts to nodes"
    ):
...
def simplify_graph_with_text(
    ...
    save_graph_with_text_as_JSON(
        new_graph, data_dir=data_dir_output, graph_name=graph_path
    )
